...
use rand::rngs::OsRng;
use rsa::{
    PaddingScheme, PrivateKeyEncoding, PrivateKeyPemEncoding, PublicKey, PublicKeyEncoding,
    PublicKeyPemEncoding, RSAPrivateKey, RSAPublicKey,
};
use shared::utils;
...
fn gen_rsa(path: &str) {
    ...
    private_key_file
        .write_all(&private_key.to_pkcs8().unwrap())
        .unwrap();

    // also store key.pem
    private_key_file = File::create(format!("{}.pem", path)).unwrap();
    private_key_file
        .write_all(&private_key.to_pem_pkcs8().unwrap().as_bytes())
        .unwrap();

    let mut public_key_file = File::create(pub_key_path).unwrap();
    public_key_file
        .write_all(&public_key.to_pkcs1().unwrap())
        .unwrap();
}
...
fn sign(pub_key_path: &str, path: &str) {
    ...
    println!(
        "sha256: {}",
        hex::encode(utils::sha256(&read(path).unwrap()))
    );
...
fn verify(private_key_path: &str, path: &str, sign: &str) {
    ...
    println!(
        "is sign ok? {}",
        utils::verify_file_sign(path, sign, &private_key).unwrap()
    );
}
